...
export function validateCursorCalibration(entry) {
  ...
  const samples = (entry.samples ?? []).filter((sample) => Number(sample.dashboardCostUsd) > 0
    && sample.resolvedModel === entry.resolvedModel
...
export function priceCursorRun(config, requestedModel, resolvedModel, serviceTier, usage) {
  ...
  if (!calibration.verified) return { cost: null, status: 'unverified', reason: calibration.reason, rateCard: entry?.rateCard ?? null };
  if (!resolvedModel || resolvedModel !== entry.resolvedModel) {
    return { cost: null, status: 'unverified', reason: 'resolved-model-mismatch', rateCard: entry.rateCard };
  }
  if (!serviceTier || serviceTier !== entry.serviceTier) {
    return { cost: null, status: 'unverified', reason: 'service-tier-unobserved-or-mismatched', rateCard: entry.rateCard };
  }
  return {
...
export function assessCursorEligibility(config, requestedModel, resolvedModel, serviceTier, {
  boundaryViolation = false, needsToolEvidence = false, argsVisible = null,
} = {}) {
  ...
  if (resolvedModel !== expected.resolvedModel) return { eligible: false, reason: 'resolved-model-mismatch' };
  if (serviceTier !== expected.serviceTier) {
