...
export async function handleTransactionStrategy(
  ...
  abi: Abi,
  clientAddress: Hex,
): Promise<TransactionStrategyResponse> {
  ...
  if (newOrProposal === 'propose' && !delegates.includes(clientAddress)) {
    throw nodeLogger.formatError(['You are neither an owner or a delegate of this Safe']);
  }

  const selections = pendingTxs.results.reduce((acc, tx) => {
    if (tx.to === transaction.to && tx.data?.startsWith(transaction.data?.slice(0, 10))) {
      const data = decodeFunctionData({ abi, data: tx.data as Hex });
  ...
  if (exactMatch) {
    if (exactMatch.signed) {
      nodeLogger.log(`Transaction ${exactMatch.safeTxHash} matches exactly and is already confirmed. Skipping.`);
      return { action: 'skip', hash: exactMatch.safeTxHash };
    }
    nodeLogger.log(`Transaction ${exactMatch.safeTxHash} matches exactly. ${newOrProposal === 'propose' ? 'Already proposed.' : 'Confirming.'}`);
  ...
  if (selections.length > 0) {
    ...
    let retryCount = 0;
    while (retryCount < 3) {
      const answer = await nodeLogger.prompt(promptMessage);
      if (answer === 's') {
        return { action: 'skip' };
      } else if (answer === 'n') {
      ...
      nodeLogger.log(`Invalid choice, please try again.`);
      retryCount++;
    }
    throw nodeLogger.formatError(['Retry count exceeded'])
  }
  return { action: newOrProposal };
}
