...
export async function getLedgerAccount(network: Network, accountIndex: number = 0): Promise<AvalancheAccount> {
  ...
  const { address: pChainAddressRaw } = await appAva.getAddressAndPubKey(
    ledger.getPChainPath(accountIndex), false, hrp
  );
  // Normalise to ensure we have the P- prefix
  const pChainAddress = pChainAddressRaw.startsWith('P-') ? pChainAddressRaw : `P-${pChainAddressRaw}`;

  // EVM account – all operations use the ETH derivation path
  const evmAccount = toAccount({
    ...
    async signTransaction(transaction) {
      ...
      const signature = await appAva.signEVMTransaction(
        ledger.getETHPath(accountIndex), serializedTx.slice(2), resolution
      ).catch((error: any) => {
    ...
    async signTypedData({ domain, types, primaryType, message }: any) {
      ...
      const signature = await appAva.signEIP712HashedMessage(
        ledger.getETHPath(accountIndex), domainSepHex, structHashHex
      ).catch((error: any) => {
        if (error.message.includes('0x6986')) {
          logger.error('Error: User rejected the transaction');
          process.exit(1);
  ...
  // P/X-chain account – signs via the P-chain derivation path
  ...
  return {
    ...
    request: async ({ method, params }: { method: any; params: any }) => {
      switch (method) {
        ...
        case 'personal_sign': {
          // Safe sends the raw hash; sign it directly on the ETH path
