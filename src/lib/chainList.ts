...
export async function setCustomChainRpcUrl(rpcUrl: string) {
  ...
  let networkIDresp = "5"
  try {
    networkIDresp = (await infoApi.getNetworkId()).networkID
  } catch (error) {
    console.warn("Warning: Custom RPC seems to be down or the infoAPI is not available, please check your RPC URL. We fallback to fuji chain.", error);
  }
  const chainId = await getChainId(rpcUrl)
  chainList.custom = defineChain({
    ...chainList.custom,
    testnet: networkIDresp === "1" ? false : true,
    network: networkIDresp === "1" ? 'mainnet' : 'fuji',
    id: Number(chainId),
    rpcUrls: {
      default: { http: [rpcUrl] },
    },
