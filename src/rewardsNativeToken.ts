 ...
 */
export async function getRewardsAmountFromEpoch(
  rewards: SafeSuzakuContract['RewardsNativeToken'],
  epoch: number
) {
  const result = await rewards.read.getEpochRewards(
    [epoch]
  );

  logger.log(`Rewards amount per token for epoch ${epoch}: ${result.toString()}`);
...
export async function getProtocolRewards(
  ...
  const rewardsAmount = await rewards.read.protocolRewards() as bigint;
...
/**
 * Gets last claimed epoch for protocol (Removed from native token rewards)
 */
